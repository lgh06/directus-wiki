...
If your server is not compatible with the above requirements please contact your host to upgrade. Also, you can optionally install the following for added functionality:
...
* **Filesystem**
  * **Local Adapter**
    ...
    // BASE_PATH constant doesn't end with trailing slash so you need to start with a trailing slash
    // You could also use the absolute path directly here
    // If you are using realpath make sure the directory actually exists
    'root' => BASE_PATH . '/media',
    // This is the url where all the media will be pointing to
    // here all assets will be (yourdomain)/media
    // same with thumbnails (yourdomain)/media/thumbs
    // It can also be used a whole url
    // Ex: 'root_url' => 'https://assets.yourdomain.com/media',
    'root_url' => '/media',
